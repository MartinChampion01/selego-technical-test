        ...
        {activeProjects.map((hit) => {
          return (
            <div
              ...
              onClick={() => {   //C6F8DB
                if (hit._id){
                history.push(`/project/${hit._id}`)
              }}}

              className={`flex justify-between flex-wrap p-3 border border-black ${hit.activity === "archived" ? `bg-[#C6F8DB]` : `bg-[#F9FBFD]`} rounded-[16px] mt-3 cursor-pointer`}>
