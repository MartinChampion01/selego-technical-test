...
export default function ProjectView() {
  ...
  const handleArchive = async () => {
    try {
      await api.post(`/project/${ id}/archive`);
      setArchived(true);
    } catch(error) {
  ...
  return (
    <React.Fragment>
      <div className="pl-20 pt-24 pb-4 w-[98%]">
        <div className={` ${project[0].activity === "archived" ? `bg-[#C6F8DB]` : `bg-[#F9FBFD]`} border border-[#E5EAEF] py-3 rounded-[16px]`}>
          <div className="flex justify-between px-3 pb-2  border-b border-[#E5EAEF]">
            ...
            {archived === false &&
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="border !border-green-500 text-[green] py-[7px] px-[20px] bg-[#FFFFFF] rounded-[16px]">
                  Archive
                </button>
              </div>
            }
            {archived === true &&
            <div className="flex items-center gap-2">
            <span className="border !border-green-500 text-[green] py-[7px] px-[20px] bg-[#FFFFFF] rounded-[16px]">
              Archived
            </span>
          </div>
            }
...
//bug 4 : l.75 : toString() was used to display the name of the project but project.name already is a string. This was preventing us from navigate to the detail project window
//bug 5 : l.76+84+87+90 : the project we got was a table so we should put project[0]. to get values. I think the purpose of this bug was to understand why here the API was sending a table and to fix that but I don't have time anymore
