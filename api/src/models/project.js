...
const Schema = new mongoose.Schema({
  ...
  status: { type: String },
  activity: { type: String, enum: ["active", "archived"], default:"active"},
});

Schema.statics.findByAnyUpdate = async function (projectId, updateData) {
  try {
    return this.findByIdAndUpdate(projectId, updateData, { new: true });
  } catch (error) {
    throw error;
  }
};
